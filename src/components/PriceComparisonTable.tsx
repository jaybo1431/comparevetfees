    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="p-3 sm:p-4 border-b border-gray-100 space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-500">Compare costs across {practices.length} practices</p>
        </div>
        <div className="flex items-center gap-2">
          <select
            value={selectedProcedure}
            onChange={(e) => setSelectedProcedure(e.target.value as PriceKey)}
            className="text-xs border border-gray-200 rounded-lg px-2 py-2 bg-white focus:outline-none focus:border-blue-500 flex-1 min-w-0"
          >
            {PRICE_KEYS.map((key) => (
      ...
      <div className="divide-y divide-gray-50">
        {sorted.map((practice, idx) => {
          ...
          return (
            <div key={practice.slug} className="flex items-start px-3 sm:px-4 py-2.5 sm:py-3 hover:bg-gray-50 transition">
              ...
              <div className="flex-1 min-w-0 ml-2 sm:ml-3">
                <p className="text-xs sm:text-sm font-medium text-gray-900">{practice.name}</p>
                <p className="text-[10px] sm:text-xs text-gray-400">{practice.town} &middot; {practice.isIndependent ? "Independent" : practice.parentGroup}</p>
              </div>
